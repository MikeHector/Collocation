function [ ] = animateSLIP(opt_results, record_video)
%plot_slip gives visualization for slip results
%   Feed it an opt_results struct and watch the pretty output
     if record_video==1
        v=VideoWriter('Baseline','MPEG-4');
        v.FrameRate=10;
        open(v);
     end

    s = opt_results;
    k = s.param(s.collParam.modelParamList == 'k');
    c = s.param(s.collParam.modelParamList == 'c');
    transmission = s.param(s.collParam.modelParamList == 'transmission');
    transmission_ankle = s.param(s.collParam.modelParamList == 'transmission_ankle');
    lf = s.param(s.collParam.modelParamList == 'lf');
    Fs = k * (s.r0 - s.r);
    ...
    ax11.YLim = [min(ankleF), max(ankleF)];
