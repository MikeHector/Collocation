...
% decision variable
function [optimized, opt_res] = RUN_COL(seed, modelParam, collParam ) %TD_angle should be nan unless it is dictated
...
%     %Cost function
    cost_func = @(x) SymObjFunc2(x, modelParam);
% 
...
%     dr0 = ub(1+5*cp:6*cp); %dr0
    ...
    collParam.options = optimoptions('fmincon','Display','iter','MaxFunctionEvaluations',5000,...
                    'MaxIterations',3000, 'ConstraintTolerance', 1e-5,...
                    'UseParallel', true, 'OptimalityTolerance',1e-2, ...
