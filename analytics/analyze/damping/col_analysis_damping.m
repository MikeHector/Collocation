...
while results_sorted_var{i}.param(varInd) < varmaxplot
   ...
   if results_sorted_var{i}.collParam.flag > 0
        ...
        mgd = m * results_sorted_var{i}.param(10) * (results_sorted_var{i}.x(end) - results_sorted_var{i}.x(1));
        Tcycle = results_sorted_var{i}.Tstance + results_sorted_var{i}.Tflight;
        Tflight = results_sorted_var{i}.Tflight;
        disp(['Energy of cycle is '...
            num2str(results_sorted_var{i}.cost * mgd), ' J'...
            ' Cycle Time = ' num2str(Tcycle), ' seconds',...
...
%         an4.YData = ankleF(1:results_sorted_var{i}.collParam.Nstance);
        an4.XData = time(1:results_sorted_var{i}.collParam.Nstance)/time(results_sorted_var{i}.collParam.Nstance);
...
%         xdist = results_sorted_var{i}.x(end)-results_sorted_var{i}.x(1)
        ...
        drawnow
        title1.String = ['Energy Required when \zeta = ', num2str(zeta,2)];
        pause(.005)
        if record_video==1
            F=getframe(gcf);
            writeVideo(v,F);
