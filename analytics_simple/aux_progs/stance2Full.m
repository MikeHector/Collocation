...
opt.t = [linspace(-tTemp(end),0,N), opt.t, linspace(opt.t(end),opt.t(end)+tApex(end),N)] + tTemp(end);
opt.x = [xf, opt.x', xlo];
...
opt.stanceStartTime = opt.t(N);
opt.stanceEndTime = opt.t(2*N);
