function [dv, PsymStruc, v, Example] = makeParamSymNew(CollocationParameters, C)
...
%them
    ...
    if C.stanceCollocation == 1
        ...
        numberOfDV = (2* CollocationParameters.dof +...
            CollocationParameters.cntrl_dof) * (CollocationParameters.N) + extraDV;
        %state+control for whole traj; 
        dv = sym('x', [1 numberOfDV], 'real')'; %all my decision vars (one hop with flight)
    %     x1 = x(1)... x80 = x(80); x81 = y(1)... Tstance, apex height,
    %     %apex vel; extraDV = Tstance, apexHeight, apexVel ...etc...
    end
...
%     dv_num_block(end+1,1:2) = [dv_num_list(end-1), dv_num_list(end)];
    ...
    dv_num_block(end,1:extraDV) = dv_num_list(end-(extraDV-1):end);
