...
%     dr0 = ub(1+5*cp:6*cp); %dr0
    ...
    ub(8*cp+2) = 5; %apex height
    ub(8*cp+3) = modelParam(13); %apex velocity
...
%     dr0 = ub(1+5*cp:6*cp); %dr0
    ...
    lb(8*cp+2) = .2; %apex height
    lb(8*cp+3) = modelParam(13); %apex velocity
...
%     assert( seed(8*cp+collParam.Nflight +2) < collParam.tflightmax && seed(8*cp+collParam.Nflight +2) > collParam.tflightmin, 'time flight error')
    ...
    opt_res.apexHeight = optimized(8*cp+2);
    opt_res.apexVelocity = optimized(8*cp+3);
    ...
    opt_res.param(12) = opt_res.apexHeight;
    opt_res.param(13) = opt_res.apexVelocity;
end