...
clear;
c = colls;
c.saveDir = 'C:\Users\DRL-Valkyrie\Google Drive\CollocationResults\absIntFVobj_sweep_apex_height_alpha';
...
g.min = 0;
g.max = .12; %Checked!
g.courseMax = .12/50; %Lets do same as apex height
listLow = linspace(g.min,g.nom,ceil((g.nom-g.min)/g.courseMax)+1);
listHigh = linspace(g.nom,g.max,ceil((g.max-g.nom)/g.courseMax)+1);
g.list = [listLow listHigh(2:end)];
c.grid{2} = g;
...
copyfile('opt_ankle_seed_fancy.mat',strcat(c.saveDir,'\opt_ankle_seed.mat'));
copyfile('opt_noAnkle_seed_fancy.mat',strcat(c.saveDir,'\opt_noAnkle_seed.mat'));
c.utility.loadSavesNeeded = 1;
c = c.loadSaves;
